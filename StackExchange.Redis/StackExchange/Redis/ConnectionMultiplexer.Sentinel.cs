    ...
    public partial class ConnectionMultiplexer
    {
        ...
        internal void InitializeSentinel(Action<string> log)
        {
            ...
            if (sub.SubscribedEndpoint("+switch-master") == null)
            {
                ...
                {
                    string[] messageParts =
                        ((string)message).Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
                    // We don't care about the result of this - we're just trying
                    _ = Format.TryParseEndPoint(
                        messageParts[1],
                        messageParts[2],
                        out var switchBlame);
            ...
            ConnectionFailed += (sender, e) =>
                ...
                Reconfigure(first: false, reconfigureAll: true, log, e.EndPoint, "Lost sentinel connection", false);
        ...
        public static ConnectionMultiplexer SentinelConnect(string configuration, Action<string> log = null) =>
        ...
        /// <param name="log">The <see cref="TextWriter"/> to log to.</param>
        public static ConnectionMultiplexer SentinelConnect(ConfigurationOptions configuration, Action<string> log = null)
        {
        ...
        /// <param name="configuration">The configuration options to use for this multiplexer.</param>
        /// <param name="log">The <see cref="TextWriter"/> to log to.</param>
        private static ConnectionMultiplexer SentinelPrimaryConnect(ConfigurationOptions configuration,
        ...
        public ConnectionMultiplexer GetSentinelMasterConnection(ConfigurationOptions config, Action<string> log = null)
        {
            if (ServerSelectionStrategy.ServerType != ServerType.Sentinel)
            {
                throw new RedisConnectionException(ConnectionFailureType.UnableToConnect,
                    "Sentinel: The ConnectionMultiplexer is not a Sentinel connection. Detected as: " +
                    ServerSelectionStrategy.ServerType);
            }
            ...
            lock (sentinelConnectionChildren)
            {
                if (sentinelConnectionChildren.TryGetValue(serviceName, out var sentinelConnectionChild) &&
                    !sentinelConnectionChild.IsDisposed)
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Roslynator",
            "RCS1075:Avoid empty catch clause that catches System.Exception.", Justification = "We don't care.")]
        internal void OnManagedConnectionRestored(object sender, ConnectionFailedEventArgs e)
        {
            if (!(sender is ConnectionMultiplexer connection))
            {
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (connection.GetServer(e.EndPoint).IsSlave ||
                        e.EndPoint != connection.currentSentinelPrimaryEndPoint)
                    {
            ...
            if (connection.sentinelPrimaryReconnectTimer == null)
            {
                connection.sentinelPrimaryReconnectTimer = new Timer(_ =>
                {
                    ...
                    finally
                    {
                        try
                        {
                            connection.sentinelPrimaryReconnectTimer?.Change(TimeSpan.FromSeconds(1),
        ...
        internal EndPoint GetConfiguredPrimaryForService(string serviceName) =>
            GetServerSnapshot() // same as GetServerSnapshot, but without forcing span
                .Where(s => s.ServerType == ServerType.Sentinel)
                .Select(s =>
                {
                    try
                    {
                        return GetServer(s.EndPoint).SentinelGetMasterAddressByName(serviceName);
                    }
                    catch
                    {
                        return null;
        ...
        internal EndPoint[] GetReplicasForService(string serviceName) =>
            GetServerSnapshot() // same as GetServerSnapshot, but without forcing span
                ...
                .Select(s =>
                {
                    try
                    {
                        return GetServer(s.EndPoint).SentinelGetReplicaAddresses(serviceName);
                    }
                    catch
                    {
                        return null;
        ...
        /// <param name="logger">The writer to log to, if any.</param>
        internal void SwitchPrimary(EndPoint switchBlame, ConnectionMultiplexer connection, Action<string> logger = null)
        {
            if (!(connection.RawConfig.ServiceName is string serviceName))
            {
                logger?.Invoke("Service name not defined.");
                return;
            }
            ...
            if (!connection.servers.Contains(newPrimaryEndPoint))
            {
                ...
                connection.Reconfigure(first: false, reconfigureAll: false, logger, switchBlame,
                    $"Primary switch {serviceName}", false, CommandFlags.PreferMaster);
        ...
        internal void UpdateSentinelAddressList(string serviceName)
        {
            var firstCompleteRequest = GetServerSnapshot()
                ...
                .Select(s =>
                {
                    try
                    {
                        return GetServer(s.EndPoint).SentinelGetSentinelAddresses(serviceName);
                    }
                    catch
                    {
                        return null;
            ...
            if (hasNew)
            {
                ...
                Reconfigure(first: false, reconfigureAll: true, null, EndPoints[0], "Updating Sentinel List",
                    false);
            }
        }
    }
